...
In a static medium, we found that a particular implementation well reproduce the semi-analytic calculations of the parton radiative energy loss and reasonably explains the gluon spectra.
...
\end{eqnarray}
Because we do not use quantum statistics in the Monte Carlo generators, we have dropped the Bose enhancement and the Pauli blocking factors compared to the original formula.
...
So the spectrum is proportional to $\sqrt{\hat{q}/\omega}$, corroborating the previous qualitative arguments.
In the ``modified rescattering" approach, the factor in the second line is included in the formation time redefinition in Equation \ref{eq:formation-time-def}.
...
Next we tested the path-length $L$ dependence of the energy loss $\Delta E$ of a quark with $E = 106$ GeV in a finite medium.
...
Here $L_c^0 = \sqrt{E/\hat{q}_0}$ is an estimate of the critical path length below which one expects a non-linear $L$-dependence and $\hat{q}_0 = C_A \alpha_s T m_D^2$.
...
Finally, we validate the gluon radiation spectrum with the ``Modified Rescattering" approach. 
...
\label{fig:Bjorken}
\end{figure}
Before applying this ``Modified rescattering" implementation to the heavy ion collisions, there are still a few issues we need to study and address.
...
In a more realistic event-by-event hydrodynamics simulations, the temperature profile are much more complicated.
Initial condition fluctuation creates medium hot spots and radial expansion boosted the local fluid rest frame.
...
For elastic scattering vertices, $\alpha_s^{\textrm{el}}$ is evaluated at the momentum transfer squared. 
This is already the feature of {\tt Lido} with running coupling.
For splitting vertices,  $\alpha_s^{\textrm{rad}}$ is evaluated at the final gluon transverse momenta squared,
...
The dash-dotted lines are running coupling calculations where the $\alpha_s^{\textrm{el}}$ is evaluated at $t$-channel momentum transfer and the $\alpha_s^{\textrm{rad}}$ at $k_{T,0}^2$.
The dashed lines are also running coupling calculations but evaluate $\alpha_s^{\textrm{rad}}$ at $Q^2 = k_{\perp,n}^2$ through the modified acceptance $p'$.
The running coupling effect results in a reduction of the energy loss compared to the references.
...
Finally, we put back the mass effects to treat the heavy quark sector. These include the massive particle kinematics, applying the following substitution in the formation time,
\begin{eqnarray}
(1-x)m_g^2 \rightarrow x^2M^2 + (1-x)m_g^2
\end{eqnarray}
and implementing the so-called ``dead-cone" effect that suppresses collinear radiations with angles $\theta \sim k_\perp/k < M/E$. 
...
To reduce the theory uncertainty introduced in the numerical implementation of the perturbative QCD transport of hard partons inside a quark-gluon plasma, we studied three different Monte-Carlo implementations and compare them to semi-analytic theory calculations.
